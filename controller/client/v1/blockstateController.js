...
const softDeleteBlockstate = async (req,res) => {
  try {
    ...
    let updatedBlockstate = await dbService.updateOne(Blockstate, query, updateBody);
    if (!updatedBlockstate){
      return res.recordNotFound();
    }
    return res.success({ data:updatedBlockstate });
...
const deleteBlockstate = async (req,res) => {
  try { 
    ...
    const query = { _id:req.params.id };
    const deletedBlockstate = await dbService.deleteOne(Blockstate, query);
...
const deleteManyBlockstate = async (req, res) => {
  try {
    ...
    const deletedBlockstate = await dbService.deleteMany(Blockstate,query);
    if (!deletedBlockstate){
      return res.recordNotFound();
    }
    return res.success({ data :{ count :deletedBlockstate } });
  } catch (error){
    return res.internalServerError({ message:error.message }); 
  }
};
...
const softDeleteManyBlockstate = async (req,res) => {
  try {
    ...
    let updatedBlockstate = await dbService.updateMany(Blockstate,query, updateBody);
    if (!updatedBlockstate) {
      return res.recordNotFound();
    }
    return res.success({ data:{ count :updatedBlockstate } });
