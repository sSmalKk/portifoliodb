...
const softDeleteItem = async (req,res) => {
  try {
    ...
    let updatedItem = await dbService.updateOne(Item, query, updateBody);
    if (!updatedItem){
      return res.recordNotFound();
    }
    return res.success({ data:updatedItem });
...
const deleteItem = async (req,res) => {
  try { 
    ...
    const query = { _id:req.params.id };
    const deletedItem = await dbService.deleteOne(Item, query);
    if (!deletedItem){
      return res.recordNotFound();
    }
...
const deleteManyItem = async (req, res) => {
  try {
    ...
    const deletedItem = await dbService.deleteMany(Item,query);
    if (!deletedItem){
      return res.recordNotFound();
    }
    return res.success({ data :{ count :deletedItem } });
  } catch (error){
    return res.internalServerError({ message:error.message }); 
  }
};
...
const softDeleteManyItem = async (req,res) => {
  try {
    ...
    let updatedItem = await dbService.updateMany(Item,query, updateBody);
    if (!updatedItem) {
      return res.recordNotFound();
    }
    return res.success({ data:{ count :updatedItem } });
