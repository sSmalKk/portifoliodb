...
/**
 * @description : deactivate document of Chat from table by id;
 * @param {Object} req : request including id in request params.
 * @param {Object} res : response contains updated document of Chat.
 * @return {Object} : deactivated Chat. {status, message, data}
 */
const softDeleteChat = async (req,res) => {
  try {
    ...
    let updatedChat = await deleteDependentService.softDeleteChat(query, updateBody);
    if (!updatedChat){
      return res.recordNotFound();
    }
    return res.success({ data:updatedChat });
...
/**
 * @description : delete document of Chat from table.
 * @param {Object} req : request including id as req param.
 * @param {Object} res : response contains deleted document.
 * @return {Object} : deleted Chat. {status, message, data}
 */
const deleteChat = async (req,res) => {
  try {
    ...
    if (req.body.isWarning) { 
      deletedChat = await deleteDependentService.countChat(query);
    } else {
      deletedChat = await deleteDependentService.deleteChat(query);
    }
    if (!deletedChat){
      return res.recordNotFound();
    }
...
/**
 * @description : delete documents of Chat in table by using ids.
 * @param {Object} req : request including array of ids in request body.
 * @param {Object} res : response contains no of documents deleted.
 * @return {Object} : no of documents deleted. {status, message, data}
 */
const deleteManyChat = async (req, res) => {
  try {
...
/**
 * @description : deactivate multiple documents of Chat from table by ids;
 * @param {Object} req : request including array of ids in request body.
 * @param {Object} res : response contains updated documents of Chat.
 * @return {Object} : number of deactivated documents of Chat. {status, message, data}
 */
const softDeleteManyChat = async (req,res) => {
  try {
    let ids = req.body.ids;
    if (!ids || !Array.isArray(ids) || ids.length < 1) {
      return res.badRequest();
