...
const softDeleteSize = async (req,res) => {
  try {
    ...
    let updatedSize = await deleteDependentService.softDeleteSize(query, updateBody);
    if (!updatedSize){
      return res.recordNotFound();
    }
    return res.success({ data:updatedSize });
...
const deleteSize = async (req,res) => {
  try {
    ...
    if (req.body.isWarning) { 
      deletedSize = await deleteDependentService.countSize(query);
    } else {
      deletedSize = await deleteDependentService.deleteSize(query);
    }
    if (!deletedSize){
      return res.recordNotFound();
    }
